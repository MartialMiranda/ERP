...
async function execute(equipoId, datosActualizados, usuarioId) {
  try {
    ...
    if (!equipo) {
      logger.warn(`Intento de actualizar equipo inexistente o sin permisos: ${equipoId}`);
    ...
    if (datosActualizados.lider_id !== undefined) {
      ...
      if (!liderExiste) {
        logger.warn(`Nuevo líder de equipo no encontrado: ${datosActualizados.lider_id}`);
    ...
    await db.tx(async t => {
      ...
      if (datosActualizados.lider_id !== undefined && datosActualizados.lider_id !== equipo.lider_id) {
        ...
        logger.info(`Líder de equipo actualizado: ${equipo.lider_id} -> ${datosActualizados.lider_id}`);
      }
      
      // Actualizar miembros del equipo si se especifican
      if (datosActualizados.miembros !== undefined && Array.isArray(datosActualizados.miembros)) {
        ...
        const miembrosNuevos = datosActualizados.miembros.filter(id => 
        ...
        const miembrosEliminar = idsActuales.filter(id => 
        ...
        if (miembrosEliminar.length > 0) {
          ...
          logger.info(`Miembros eliminados del equipo: ${miembrosEliminar.join(', ')}`);
        }
      }
    });
